...
<html>
    <head>
        ...
        <title>Attendance Monitor - E-Sukarelawan</title>
        <style>
    ...
    <body>
        ...
        <div class="nav-bar">
            ...
            <a href="org-view-attendance.jsp" class="active">Attendance</a>
            <a href="org-history.jsp" >History</a>
            <a href="../LogoutServlet" class="nav-logout">Logout</a>
        </div>
        <div class="page-wrap">
            <div class="page-header">
                <h1>Registration &amp; Attendance Monitor</h1>
                <p>Track which students registered and who successfully claimed credit.</p>
            </div>

            <div class="card">
                <div class="card-title"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg> Filter by Event</div>
                <form action="org-view-attendance.jsp" method="GET" style="display:flex;gap:10px;align-items:end;flex-wrap:wrap;">
                    <div style="flex:1;min-width:200px;">
                        <label for="eventId" style="display:block;font-size:0.82rem;font-weight:600;color:var(--c-muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.5px;">Select Campaign</label>
                        <select id="eventId" name="eventId" onchange="this.form.submit()">
                            <option value="">-- Choose Campaign --</option>
                            <%
                                try {
                                    EventDAO evDAO = new EventDAO();
                                    List<event> myEvents = evDAO.getEventsByOrgId(currentOrg.getOrgId());
                                    String selectedId = request.getParameter("eventId");
                                    if (myEvents != null) { for (event ev : myEvents) {
                                        boolean isSel = selectedId != null && selectedId.equals(String.valueOf(ev.getEventId()));
                            %>
                            <option value="<%= ev.getEventId()%>" <%= isSel ? "selected" : ""%>><%= ev.getEventName()%> (<%= ev.getEventDate()%>)</option>
                            <% } } } catch (Exception ex) { out.println("<option>Error loading</option>"); } %>
                        </select>
                    </div>
                    <button type="submit" class="btn btn-primary">Query Roster</button>
                </form>
            </div>
            ...
            %>
            <div class="card">
                <div class="card-title"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> Roster: <%= selectedEvent.getEventName()%></div>
                <div class="card-subtitle">Credits: <strong><%= selectedEvent.getEventHour()%> Hours</strong> &bull; Code: <code><%= selectedEvent.getSecretCode()%></code></div>
                <div class="table-wrap">
                    <table><thead><tr><th>Reg ID</th><th>Student ID</th><th>Student Name</th><th>Course</th><th>Status</th></tr></thead>
                    <tbody>
                        <%
                        ...
                        %>
                        <tr><td colspan="5" style="text-align:center;padding:24px;color:var(--c-muted);">No students registered yet.</td></tr>
                        <% } else { for (registration reg : rosters) { volunteer student = volunteerDAO.getVolunteerById(reg.getVolunteerId()); if (student != null) { %>
                        <tr>
            ...
            <%      } else { out.println("<div class=\"alert alert-error\"><strong>Access denied for this event.</strong></div>"); }
                    } catch (NumberFormatException nfe) { out.println("<div class=\"alert alert-error\"><strong>Invalid query parameter.</strong></div>"); }
                } else { %>
            <div class="card" style="text-align:center;padding:48px;color:var(--c-muted);">
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="margin-bottom:12px;opacity:0.4;"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
                <p>Select a campaign above and click <strong>Query Roster</strong> to view student registrations.</p>
            </div>
            <% } %>
        </div>
    </body>
