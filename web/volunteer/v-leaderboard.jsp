...
<html>
    ...
    <body>
        ...
        <div class="nav-bar">
            ...
            <a href="v-leaderboard.jsp" class="active">Leaderboard</a>
            <a href="../LogoutServlet" class="nav-logout">Logout</a>
        </div>
        <div class="page-wrap">
            <div class="page-header">
                <h1>Global Social Impact Leaderboard</h1>
                <p>Honoring our top contributors generating community hours across E-Sukarelawan.</p>
            </div>

            <div class="card">
                <div class="card-title"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg> Top Volunteer Leaders</div>
                <div class="table-wrap">
                    <table><thead><tr><th>Rank</th><th>Full Name</th><th>Course</th><th>Verified Hours</th></tr></thead>
                    <tbody>
                        <%
                            try {
                        ...
                        %>
                        <tr><td colspan="4" style="text-align:center;padding:24px;color:var(--c-muted);">No leaderboard data yet.</td></tr>
                        <% } else { int ranking = 1; for (volunteer v : sortedVolunteers) { boolean isMe = (v.getVolunteerId() == currentVolunteer.getVolunteerId()); %>
                        <tr<%= isMe ? " class=\"highlight\"" : "" %>>
                            <td><span class="badge badge-rank">#<%= ranking++ %></span></td>
                            <td><strong><%= v.getFullName() %></strong> <%= isMe ? "<span class=\"badge badge-info\">You</span>" : "" %></td>
                            <td><%= v.getCourse() %></td>
                            <td><span class="badge badge-success"><%= v.getTotalHours() %> Hours</span></td>
                        </tr>
                        <% } } } catch (Exception ex) { %>
                        <tr><td colspan="4" style="color:var(--c-danger);">Error: <%= ex.getMessage() %></td></tr>
                        <% } %>
                    </tbody></table>
                </div>
            </div>
